...
public class diff_match_patch {
	...
	protected static class LinesToCharsResult {
		...
		protected LinesToCharsResult(final String chars1, final String chars2,
	...
	public LinkedList<Diff> diff_main(final String text1, final String text2,
		...
		if (this.Diff_Timeout <= 0) {
			...
			deadline = System.currentTimeMillis() + (long) (this.Diff_Timeout * 1000);
		}
	...
	private LinkedList<Diff> diff_main(String text1, String text2,
			final boolean checklines, final long deadline) {
	 ...
	 * @return Linked List of Diff objects.
	 */
	private LinkedList<Diff> diff_compute(final String text1, final String text2,
			final boolean checklines, final long deadline) {
		...
		if (i != -1) {
			...
			final Operation op = (text1.length() > text2.length()) ?
		...
		final String[] hm = this.diff_halfMatch(text1, text2);
		if (hm != null) {
			...
			final LinkedList<Diff> diffs_a = this.diff_main(text1_a, text2_a,
					checklines, deadline);
			final LinkedList<Diff> diffs_b = this.diff_main(text1_b, text2_b,
					checklines, deadline);
			// Merge the results.
			diffs = diffs_a;
	...
	private LinkedList<Diff> diff_lineMode(String text1, String text2,
		...
		final LinesToCharsResult b = this.diff_linesToChars(text1, text2);
		text1 = b.chars1;
		text2 = b.chars2;
		final List<String> linearray = b.lineArray;

		final LinkedList<Diff> diffs = this.diff_main(text1, text2, false, deadline);
		...
		while (thisDiff != null) {
			...
			case EQUAL:
				...
				if (count_delete >= 1 && count_insert >= 1) {
					...
					for (final Diff newDiff : this.diff_main(text_delete, text_insert, false,
							deadline)) {
						pointer.add(newDiff);
					}
				}
	 ...
	 */
	protected LinkedList<Diff> diff_bisect(final String text1, final String text2,
			final long deadline) {
		...
		final LinkedList<Diff> diffs = new LinkedList<>();
		diffs.add(new Diff(Operation.DELETE, text1));
		diffs.add(new Diff(Operation.INSERT, text2));
		return diffs;
	}
	 ...
	 * @return LinkedList of Diff objects.
	 */
	private LinkedList<Diff> diff_bisectSplit(final String text1, final String text2,
			final int x, final int y, final long deadline) {
		...
		final LinkedList<Diff> diffs = this.diff_main(text1a, text2a, false, deadline);
		final LinkedList<Diff> diffsb = this.diff_main(text1b, text2b, false, deadline);
	...
	protected LinesToCharsResult diff_linesToChars(final String text1, final String text2) {
		...
		final Map<String, Integer> lineHash = new HashMap<>();
		...
		final String chars1 = this.diff_linesToCharsMunge(text1, lineArray, lineHash);
		final String chars2 = this.diff_linesToCharsMunge(text2, lineArray, lineHash);
	 ...
	 */
	private String diff_linesToCharsMunge(final String text, final List<String> lineArray,
	 ...
	 * @param lineArray List of unique strings.
	 */
	protected void diff_charsToLines(final LinkedList<Diff> diffs,
			final List<String> lineArray) {
		StringBuilder text;
	...
	protected String[] diff_halfMatch(final String text1, final String text2) {
		...
		final String[] hm1 = this.diff_halfMatchI(longtext, shorttext,
				(longtext.length() + 3) / 4);
		// Check again based on the third quarter.
		final String[] hm2 = this.diff_halfMatchI(longtext, shorttext,
				(longtext.length() + 1) / 2);
		String[] hm;
	...
	private String[] diff_halfMatchI(final String longtext, final String shorttext, final int i) {
		...
		while ((j = shorttext.indexOf(seed, j + 1)) != -1) {
			final int prefixLength = this.diff_commonPrefix(longtext.substring(i),
					shorttext.substring(j));
			final int suffixLength = this.diff_commonSuffix(longtext.substring(0, i),
	...
	public void diff_cleanupSemantic(final LinkedList<Diff> diffs) {
		...
		final Stack<Diff> equalities = new Stack<>();  // Stack of qualities.
	...
	public void diff_cleanupSemanticLossless(final LinkedList<Diff> diffs) {
		...
		while (nextDiff != null) {
			if (prevDiff.operation == Operation.EQUAL &&
				...
				bestScore = this.diff_cleanupSemanticScore(equality1, edit)
				...
				while (edit.length() != 0 && equality2.length() != 0
					...
					score = this.diff_cleanupSemanticScore(equality1, edit)
							+ this.diff_cleanupSemanticScore(edit, equality2);
	...
	private int diff_cleanupSemanticScore(final String one, final String two) {
		...
		final boolean blankLine1 = lineBreak1 && this.BLANKLINEEND.matcher(one).find();
		final boolean blankLine2 = lineBreak2 && this.BLANKLINESTART.matcher(two).find();
	...
	public void diff_cleanupEfficiency(final LinkedList<Diff> diffs) {
		...
		boolean changes = false;
		final Stack<Diff> equalities = new Stack<>();  // Stack of equalities.
		String lastequality = null; // Always equal to equalities.lastElement().text
		final ListIterator<Diff> pointer = diffs.listIterator();
	 ...
	 */
	public String diff_prettyHtml(final LinkedList<Diff> diffs) {
	 ...
	 */
	public LinkedList<Diff> diff_fromDelta(final String text1, final String delta)
	 ...
	 */
	public int match_main(final String text, final String pattern, int loc) {
		// Check for null inputs.
	 ...
	 * @return Best match index or -1.
	 */
	protected int match_bitap(final String text, final String pattern, final int loc) {
		assert (this.Match_MaxBits == 0 || pattern.length() <= this.Match_MaxBits)
		: "Pattern too long for this application.";

		// Initialise the alphabet.
		final Map<Character, Integer> s = this.match_alphabet(pattern);
		...
		if (best_loc != -1) {
			score_threshold = Math.min(this.match_bitapScore(0, best_loc, loc, pattern),
					score_threshold);
		...
		for (int d = 0; d < pattern.length(); d++) {
			...
			for (int j = finish; j >= start; j--) {
				...
				if ((rd[j] & matchmask) != 0) {
					final double score = this.match_bitapScore(d, j - 1, loc, pattern);
	 ...
	 */
	private double match_bitapScore(final int e, final int x, final int loc, final String pattern) {
	 ...
	 */
	protected Map<Character, Integer> match_alphabet(final String pattern) {
		final Map<Character, Integer> s = new HashMap<>();
	 ...
	 */
	protected void patch_addContext(final Patch patch, final String text) {
		if (text.length() == 0) {
			return;
		}
	 ...
	 */
	public LinkedList<Patch> patch_make(final String text1, final String text2) {
		if (text1 == null || text2 == null) {
			throw new IllegalArgumentException("Null inputs. (patch_make)");
		}
		// No diffs provided, compute our own.
		final LinkedList<Diff> diffs = this.diff_main(text1, text2, true);
		if (diffs.size() > 2) {
			this.diff_cleanupSemantic(diffs);
	 ...
	 * @return LinkedList of Patch objects.
	 */
	public LinkedList<Patch> patch_make(final LinkedList<Diff> diffs) {
		if (diffs == null) {
			throw new IllegalArgumentException("Null inputs. (patch_make)");
		}
		// No origin string provided, compute our own.
		final String text1 = this.diff_text1(diffs);
		return this.patch_make(text1, diffs);
	}
	 ...
	 * @return LinkedList of Patch objects.
	 */
	public LinkedList<Patch> patch_make(final String text1, final LinkedList<Diff> diffs) {
	 ...
	 */
	public LinkedList<Patch> patch_deepCopy(final LinkedList<Patch> patches) {
		final LinkedList<Patch> patchesCopy = new LinkedList<>();
	...
	public Object[] patch_apply(LinkedList<Patch> patches, String text) {
		...
		final String nullPadding = this.patch_addPadding(patches);
		...
		for (final Patch aPatch : patches) {
			...
			final String text1 = this.diff_text1(aPatch.diffs);
			int start_loc;
			int end_loc = -1;
			...
			if (start_loc == -1) {
				...
				if (text1.equals(text2)) {
					...
					text = text.substring(0, start_loc) + this.diff_text2(aPatch.diffs)
					+ text.substring(start_loc + text1.length());
				} else {
					// Imperfect match.  Run a diff to get a framework of equivalent
					// indices.
					final LinkedList<Diff> diffs = this.diff_main(text1, text2, false);
					if (text1.length() > this.Match_MaxBits
						...
						for (final Diff aDiff : aPatch.diffs) {
							if (aDiff.operation != Operation.EQUAL) {
								final int index2 = this.diff_xIndex(diffs, index1);
								if (aDiff.operation == Operation.INSERT) {
									// Insertion
	 ...
	 */
	public String patch_addPadding(final LinkedList<Patch> patches) {
	 ...
	 */
	public void patch_splitMax(final LinkedList<Patch> patches) {
		final short patch_size = this.Match_MaxBits;
	 ...
	 */
	public String patch_toText(final List<Patch> patches) {
		final StringBuilder text = new StringBuilder();
		for (final Patch aPatch : patches) {
			text.append(aPatch);
		}
	 ...
	 * @throws IllegalArgumentException If invalid input.
	 */
	public List<Patch> patch_fromText(final String textline)
			throws IllegalArgumentException {
		...
		final LinkedList<String> text = new LinkedList<>(textList);
		Patch patch;
		final Pattern patchHeader
